#!/usr/bin/env python3
"""
TURBO OCR ingestion - Optimized for RTX 5090 (24GB VRAM)
Uses higher batch sizes and workers to maximize GPU utilization
"""
...
# TURBO SETTINGS for RTX 5090
os.environ["MARKER_BATCH_MULTIPLIER"] = "4"  # Default is 2
...
PROGRESS_FILE = "/home/user/work/polymax/ingest_ocr_turbo_progress.json"
...
def extract_with_marker(pdf_path, converter):
    """OCR with Marker (GPU) - TURBO mode."""
...
CONCEPTS = {
    ...
    "eigenvalue": "spectral_theory", "fourier": "fourier_analysis",
    "differential": "differential_equations", "integral": "integral_calculus",
...
def main(pdf_dir):
    ...
    print(f"{'='*60}")
    print(f"TURBO OCR - RTX 5090 Optimized")
    print(f"{'='*60}")
    print(f"[{datetime.now():%H:%M}] {len(remaining)}/{len(pdfs)} PDFs to process")
    print(f"Batch multiplier: {os.environ.get('MARKER_BATCH_MULTIPLIER', '2')}")
    print(f"{'='*60}\n")
    ...
    # Load Marker models once with TURBO settings
    print("Loading Marker OCR models (GPU TURBO mode)...")
    from marker.converters.pdf import PdfConverter
    from marker.models import create_model_dict

    models = create_model_dict()
    converter = PdfConverter(artifact_dict=models)
    print("Models ready - TURBO mode active\n")
    ...
    for i, pdf in enumerate(remaining, 1):
        ...
        if not text:
            try:
                print("  → Running Marker OCR (TURBO)...", flush=True)
                text = extract_with_marker(str(pdf), converter)
                method = "marker"
        ...
        # Calculate pages/min
        elapsed = (datetime.now() - pdf_start).total_seconds()

        print(f"  Title: {title[:60]}...")
        print(f"  Chars: {len(text):,}, Chunks: {len(chunks)}, Time: {elapsed:.1f}s")
        ...
        try:
            with driver.session() as session:
                session.run("MERGE (p:Paper {title: $t}) SET p.source='ocr_turbo'", {"t": title[:200]})
                for c in concepts:
                    session.run("""
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 ingest_ocr_turbo.py /path/to/pdfs/")
        sys.exit(1)
    main(sys.argv[1])
